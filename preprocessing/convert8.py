...
import warnings, swifter, pickle, calendar, misc
...
def delivery_days_to_date(df, days) -> List:
    ...
    df['days'] = days
    results = df[['acceptance_scan_timestamp', 'days']].swifter.apply(kernel, axis=1)
...
def process(df:DataFrame, no_target=False, record_number = False):
    ...
    print("target-encoding shipment ... ")
    encoder = misc.targetEncoding()
    df = encoder.encode_shipment(df)
    print("target-encoding category_id ... ")
    df = encoder.encode_category(df)
    print("target-encoding day ... ")
    df = encoder.encode_day(df)
    print("target-encoding month ... ")
    df = encoder.encode_month(df)
    print("target-encoding weekday ... ")
    df = encoder.encode_weekday(df)
    print("target-encoding hour ... ")
    df = encoder.encode_hour(df)
