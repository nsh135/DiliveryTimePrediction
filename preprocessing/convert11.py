...
significant_features = [
    ...
    'shipment_method_id_2',
    'shipment_method_id_3',
    'shipment_method_id_4',
    'shipment_method_id_5',
    'shipment_method_id_6',
    'shipment_method_id_7',
    'shipment_method_id_8',
    'category_id_4',
    'category_id_14',
    'category_id_16',
    'category_id_18',
...
def calculate_holidays(df:DataFrame):
    ...
    def nearHoliday7_kernel(acceptance_dt):
        x = datetime.fromisoformat(acceptance_dt).date()
        for d in range(1,8):
            if x+timedelta(days=d) in federalHolidays:
                return 1
        return 0
    def nearHoliday3_kernel(acceptance_dt):
        x = datetime.fromisoformat(acceptance_dt).date()
        for d in range(1,4):
            if x+timedelta(days=d) in federalHolidays:
                return 1
    ...
    df['near_holiday_7'] = df['acceptance_scan_timestamp'].swifter.apply(nearHoliday7_kernel)
    df['near_holiday_3'] = df['acceptance_scan_timestamp'].swifter.apply(nearHoliday3_kernel)
    return df
...
all_feature_names = ['b2c_c2c', 'shipment_method_id', 'shipping_fee', 'carrier_min_estimate', 'carrier_max_estimate', 'category_id', 'item_price', 'quantity', 'weight', 'distance', 'month', 'day', 'weekday', 'hour', 'is_holiday', 'near_holiday_7', 'near_holiday_3']
...
def process(df:DataFrame, no_target=False, record_number = False):
    ...
    x = onehot_encode(x)
    print("keeping significant features")
