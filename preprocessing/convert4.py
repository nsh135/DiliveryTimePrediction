...
def calculate_distance(df:DataFrame, from_cache=True):
    ...
    if from_cache:
        ...
        assert df.shape[0]==cache_df.shape[0]
...
def calculate_delivery_days(df:DataFrame):
    def delivery_days_kernel(series):
        ...
        return abs((deliver_dt-accept_dt).days)
...
feature_names = ['b2c_c2c', 'declared_handling_days', 'shipment_method_id', 'shipping_fee', 'carrier_min_estimate', 'carrier_max_estimate', 'category_id', 'item_price', 'quantity', 'weight', 'distance', 'month', 'day', 'weekday', 'hour']
features_need_onehot = ['shipment_method_id','category_id', 'month', 'day', 'weekday']
onehot_categories = {
