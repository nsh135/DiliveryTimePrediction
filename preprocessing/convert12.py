...
def calculate_year(df:DataFrame):
    def year_kernel(acceptance_dt):
        acceptance_dt = datetime.fromisoformat(acceptance_dt)
        return acceptance_dt.year
    df['year'] = df['acceptance_scan_timestamp'].swifter.apply(year_kernel)
    return df
...
def process(df:DataFrame, no_target=False, record_number = False):
    ...
    print("calculating acceptance year ...")
    df = calculate_year(df)
