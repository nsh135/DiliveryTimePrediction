...
def calculate_delivery_days(df:DataFrame):
    def delivery_days_kernel(series):
        payment_dt, deliver_dt = series['payment_datetime'], series['delivery_date']
    ...
    days = df[['payment_datetime', 'delivery_date']].swifter.apply(delivery_days_kernel, axis=1)
    return np.array(days)


feature_names = ['b2c_c2c', 'declared_handling_days', 'shipment_method_id', 'shipping_fee', 'carrier_min_estimate', 'carrier_max_estimate', 'category_id', 'item_price', 'quantity', 'weight']
